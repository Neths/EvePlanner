    ...
    for character_api in character.apis.all():
        ...
        if character_api.last_execution:
            ...
            if (now - last_execution_date).total_seconds() < api.cache_duration:
                print(f'data {api.handler} for character {character.name} already in cache, skip call')
                continue
        ...
        if api.scope and api.scope not in token_data['scp']:
            print(f'access_token for character {character.name} do not allow call api {api.handler}')
            continue

        print(f"call handler {api.handler} for character {character.name}")
